    ...
    public class webhook:IModel
    {
        ...
        public bool Create(database database)
        {
            using (var db = database.connection)
            {
                ...
                {
                    sql = $"INSERT INTO webhook_data (id_webhook_data, email, event, ip, sg_content_type, sg_event_id, sg_machine_open, sg_message_id, timestamp, useragent, create_at) VALUES (@id_webhook_data, @email, @event, @ip, @sg_content_type, @sg_event_id, @sg_machine_open, @sg_message_id, @timestamp, @useragent, DATE_FORMAT(@create_at,'%Y-%m-%d %H:%i:%s')); ";
                }
                else
                {
                    sql = $"INSERT INTO webhook_data (id_webhook_data, email, event, ip, sg_content_type, sg_event_id, sg_machine_open, sg_message_id, timestamp, useragent, create_at) VALUES (DEFAULT, @email, @event, @ip, @sg_content_type, @sg_event_id, @sg_machine_open, @sg_message_id, @timestamp, @useragent, DATE_FORMAT(@create_at,'%Y-%m-%d %H:%i:%s')); ";
                }
        ...
        public bool Update(database database)
        {
            using (var db = database.connection)
            {
                var sql = "UPDATE webhook_data SET email = @email, event = @event, ip = @ip, sg_content_type = @sg_content_type, sg_event_id = @sg_event_id, sg_machine_open = @sg_machine_open, sg_message_id = @sg_message_id, timestamp = @timestamp, useragent = @useragent, required = @required WHERE id_webhook_data = @id_webhook_data;";
                var result = db.Execute(sql, this);
                return (result > 0);
            }
        }
